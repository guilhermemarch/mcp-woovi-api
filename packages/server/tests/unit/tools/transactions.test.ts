...
describe('Transaction Tools', () => {
  ...
  describe('get_transactions tool', () => {
    ...
    it('should pass through documented charge, pixQrCode, and withdrawal filters', async () => {
      const registeredTools: any[] = [];
      const mockServer = {
        registerTool: vi.fn((name, config, handler) => {
          registeredTools.push({ name, ...config, handler });
      ...
      const mockListTransactions = vi.fn().mockResolvedValue({
        transactions: [],
        pageInfo: { skip: 0, limit: 10, totalCount: 0, hasPreviousPage: false, hasNextPage: false },
      });

      registerTransactionTools(mockServer as any, { listTransactions: mockListTransactions } as any);
  ...
  describe('get_balance tool', () => {
    ...
    it('should validate input with Zod schema and support fresh reads', async () => {
      const registeredTools: any[] = [];
      const mockServer = {
        registerTool: vi.fn((name, config, handler) => {
          registeredTools.push({ name, ...config, handler });
      ...
      expect(() => schema.parse({})).not.toThrow();
      expect(() => schema.parse({ accountId: 'acc-123', fresh: false })).not.toThrow();
    });

     it('should call wooviClient.getBalance() with fresh reads by default', async () => {
      const registeredTools: any[] = [];
      const mockServer = {
        registerTool: vi.fn((name, config, handler) => {
          registeredTools.push({ name, ...config, handler });
     ...
     it('should return MCP-compliant response format with account balance data', async () => {
      const registeredTools: any[] = [];
      const mockServer = {
        registerTool: vi.fn((name, config, handler) => {
          registeredTools.push({ name, ...config, handler });
     ...
     it('should return nested balance values in centavos', async () => {
      const registeredTools: any[] = [];
      const mockServer = {
        registerTool: vi.fn((name, config, handler) => {
          registeredTools.push({ name, ...config, handler });
      ...
      expect(parsedResult.balance.available).toBe(50000);
    });

    it('should allow cached reads when fresh is false', async () => {
      const registeredTools: any[] = [];
      const mockServer = {
        registerTool: vi.fn((name, config, handler) => {
          registeredTools.push({ name, ...config, handler });
      ...
      registerTransactionTools(mockServer as any, { getBalance: mockGetBalance } as any);

      const getBalanceTool = registeredTools.find(t => t.name === 'get_balance');
      await getBalanceTool.handler({ accountId: 'acc_default', fresh: false });

      expect(mockGetBalance).toHaveBeenCalledWith('acc_default', { bypassCache: false });
    });
