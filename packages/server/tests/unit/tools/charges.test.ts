...
import { registerChargeTools } from '../../../src/tools/charges.js';
...
describe('Charge Tools', () => {
  ...
  describe('create_charge tool', () => {
    ...
    it('should validate input with Zod schema requiring value and correlationID', async () => {
      ...
      expect(() => schema.parse({ value: -1, correlationID: 'test-uuid-123' })).toThrow();
    });

    it('should accept documented charge creation fields beyond the minimal subset', async () => {
      const registeredTools: any[] = [];
      const mockServer = {
        registerTool: vi.fn((name, config, handler) => {
          registeredTools.push({ name, ...config, handler });
      ...
      expect(() => schema.parse({
        value: 5000,
        correlationID: 'test-uuid-123',
        expiresDate: '2026-12-31T23:59:59Z',
        fixedLocation: true,
        customer: {
          name: 'Test Customer',
  ...
  describe('list_charges tool', () => {
    ...
    it('should validate input with Zod schema for status, dates, pagination', async () => {
      ...
      expect(() => schema.parse({ startDate: 'not-a-date' })).toThrow();
    ...
    it('should call wooviClient.listCharges() with filters', async () => {
      ...
      const listChargesTool = registeredTools.find(t => t.name === 'list_charges');
      await listChargesTool.handler({ status: 'ACTIVE', skip: 10, limit: 20, subscription: 'sub-123' });
